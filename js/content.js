/* Atualidades Web, propriedades CSS e interações do primeiro quadro HTML. */
...
function openCurrentTopic(key, source = null) {
  ...
  currentRows.forEach(row => row.classList.toggle('is-active', row.dataset.current === key));
...
function closeCurrentTopic() {
  ...
  currentRows.forEach(row => row.classList.remove('is-active'));
  if (currentPreviousFocus && typeof currentPreviousFocus.focus === 'function') currentPreviousFocus.focus({ preventScroll: true });
}
...
function openCurrentConcept(key, source = null) {
  ...
  currentConceptTriggers.forEach(item => {
    const active = item.dataset.currentConcept === key;
...
function closeCurrentConcept() {
  ...
  currentConceptTriggers.forEach(item => { item.classList.remove('is-selected'); item.setAttribute('aria-pressed','false'); });
  if (currentConceptPreviousFocus && typeof currentConceptPreviousFocus.focus === 'function') currentConceptPreviousFocus.focus({ preventScroll: true });
}
...
function openCssProperty(key, source = null) {
  ...
  cssPropsConceptTriggers.forEach(item => { item.classList.remove('is-selected'); item.setAttribute('aria-pressed','false'); });
  cssPropButtons.forEach(btn => btn.classList.toggle('is-active', btn.dataset.cssprop === key));
...
function closeCssProperty() {
  ...
  cssPropButtons.forEach(btn => btn.classList.remove('is-active'));
  if (cssPropsPreviousFocus && typeof cssPropsPreviousFocus.focus === 'function') cssPropsPreviousFocus.focus({ preventScroll: true });
}
...
function openCssSelectorConcept(key, source = null) {
  ...
  cssPropsConceptTriggers.forEach(item => {
    const active = item.dataset.csspropsConcept === key;
    item.classList.toggle('is-selected', active);
    item.setAttribute('aria-pressed', active ? 'true' : 'false');
  });
...
function closeCssSelectorConcept() {
  ...
  cssPropsWorkspace.classList.remove('context-mode');
  cssPropsConceptTriggers.forEach(item => { item.classList.remove('is-selected'); item.setAttribute('aria-pressed','false'); });
  if (cssPropsConceptPreviousFocus && typeof cssPropsConceptPreviousFocus.focus === 'function') cssPropsConceptPreviousFocus.focus({ preventScroll: true });
}

cssPropButtons.forEach((button, index) => {
  button.addEventListener('click', () => openCssProperty(button.dataset.cssprop, button));
  button.addEventListener('keydown', event => {
    if (event.key === 'ArrowRight' || event.key === 'ArrowLeft' || event.key === 'ArrowDown' || event.key === 'ArrowUp') {
      event.preventDefault();
      const columns = 4;
      let next = index;
      if (event.key === 'ArrowRight') next = (index + 1) % cssPropButtons.length;
      if (event.key === 'ArrowLeft') next = (index - 1 + cssPropButtons.length) % cssPropButtons.length;
      if (event.key === 'ArrowDown') next = (index + columns) % cssPropButtons.length;
      if (event.key === 'ArrowUp') next = (index - columns + cssPropButtons.length) % cssPropButtons.length;
      cssPropButtons[next].focus();
    }
  });
});
...
function renderHtmlExample(key) {
  ...
  if (!data) return;
  htmlRows.forEach(row => row.classList.toggle('is-active', row.dataset.html === key));
...
function previewHtmlRow(row) {
  ...
  if (!data) return;
  htmlRows.forEach(item => item.classList.toggle('is-active', item === row));
  htmlDetail.innerHTML = `<strong>${data.label}:</strong> ${data.detail}`;
}
...
function openHtmlConcept(key, source) {
  ...
  htmlConceptTriggers.forEach(note => {
    const selected = note.dataset.htmlConcept === key;
    note.classList.toggle('is-selected', selected);
    note.setAttribute('aria-pressed', selected ? 'true' : 'false');
  });
  htmlConnectors.forEach(line => line.classList.toggle('is-active', line.dataset.htmlConcept === key));
...
function closeHtmlConcept() {
  ...
  htmlConceptTriggers.forEach(note => { note.classList.remove('is-selected'); note.setAttribute('aria-pressed', 'false'); });
  htmlConnectors.forEach(line => line.classList.remove('is-active'));
  if (htmlPreviousFocus && typeof htmlPreviousFocus.focus === 'function') htmlPreviousFocus.focus({ preventScroll: true });
}

htmlRows.forEach(row => {
  const choose = () => renderHtmlExample(row.dataset.html);
  row.addEventListener('mouseenter', () => previewHtmlRow(row));
  row.addEventListener('focus', () => previewHtmlRow(row));
  ...
  row.addEventListener('keydown', event => {
    ...
    if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
      event.preventDefault();
      const current = htmlRows.indexOf(row);
      const delta = event.key === 'ArrowDown' ? 1 : -1;
      htmlRows[(current + delta + htmlRows.length) % htmlRows.length].focus();
    }
  });
});
htmlConceptTriggers.forEach(trigger => trigger.addEventListener('click', () => openHtmlConcept(trigger.dataset.htmlConcept, trigger)));
htmlConnectors.forEach(line => {
  const activate = () => openHtmlConcept(line.dataset.htmlConcept, line);

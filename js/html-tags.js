/* Dados e interações do quadro de tags HTML. */
...
function openTagDetail(key, source) {
  ...
  tagButtons.forEach(button => button.classList.toggle('is-active', button === source));
...
function closeTagDetail() {
  ...
  if (tagPreviousFocus && typeof tagPreviousFocus.focus === 'function') tagPreviousFocus.focus({ preventScroll: true });
}

tagButtons.forEach((button, index) => {
  button.addEventListener('click', () => openTagDetail(button.dataset.tag, button));
  button.addEventListener('keydown', event => {
    if (event.key === 'ArrowRight' || event.key === 'ArrowLeft' || event.key === 'ArrowDown' || event.key === 'ArrowUp') {
      event.preventDefault();
      const columns = 4;
      const delta = event.key === 'ArrowRight' ? 1 : event.key === 'ArrowLeft' ? -1 : event.key === 'ArrowDown' ? columns : -columns;
      tagButtons[(index + delta + tagButtons.length) % tagButtons.length].focus();
    }
  });
});

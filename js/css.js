/* Conteúdo e funções ligadas aos quadros CSS, CSS Parte 2 e dados de propriedades. */
...
function openCssDetail(key, source) {
  ...
  cssRows.forEach(row => row.classList.toggle('is-active', row === source));
...
function closeCssDetail() {
  ...
  cssRows.forEach(row => row.classList.remove('is-active'));
  if (cssPreviousFocus && typeof cssPreviousFocus.focus === 'function') cssPreviousFocus.focus({ preventScroll: true });
}
...
function openCssConcept(key, source) {
  ...
  cssConceptTriggers.forEach(note => {
    const selected = note.dataset.cssConcept === key;
    note.classList.toggle('is-selected', selected);
    note.setAttribute('aria-pressed', selected ? 'true' : 'false');
  });
  cssConnectors.forEach(line => line.classList.toggle('is-active', line.dataset.cssConcept === key));
...
function closeCssConcept() {
  ...
  cssConceptTriggers.forEach(note => { note.classList.remove('is-selected'); note.setAttribute('aria-pressed', 'false'); });
  cssConnectors.forEach(line => line.classList.remove('is-active'));
  if (cssConceptPreviousFocus && typeof cssConceptPreviousFocus.focus === 'function') cssConceptPreviousFocus.focus({ preventScroll: true });
}

cssRows.forEach(row => {
  const choose = () => openCssDetail(row.dataset.css, row);
  ...
  row.addEventListener('keydown', event => {
    ...
    if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
      ...
      const current = cssRows.indexOf(row);
      const delta = event.key === 'ArrowDown' ? 1 : -1;
      cssRows[(current + delta + cssRows.length) % cssRows.length].focus();
    }
  });
});
cssConceptTriggers.forEach(trigger => trigger.addEventListener('click', () => openCssConcept(trigger.dataset.cssConcept, trigger)));
cssConnectors.forEach(line => {
  const activate = () => openCssConcept(line.dataset.cssConcept, line);
...
function renderCss2Topic(key, source = null) {
  ...
  if (!data) return;
  activeCss2Key = key;
  css2Rows.forEach(row => row.classList.toggle('is-active', row.dataset.css2 === key));
...
function closeCss2Reference() {
  ...
  if (css2PreviousFocus && typeof css2PreviousFocus.focus === 'function') css2PreviousFocus.focus({ preventScroll: true });
}

css2Rows.forEach(row => {
  const choose = () => renderCss2Topic(row.dataset.css2, row);
  row.addEventListener('click', choose);
  row.addEventListener('keydown', event => {
    if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); choose(); }
    if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
      event.preventDefault();
      const current = css2Rows.indexOf(row);
      const delta = event.key === 'ArrowDown' ? 1 : -1;
      css2Rows[(current + delta + css2Rows.length) % css2Rows.length].focus();
    }
  });
});

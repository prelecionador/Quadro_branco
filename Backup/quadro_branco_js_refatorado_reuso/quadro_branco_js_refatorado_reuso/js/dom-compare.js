/*
===============================================================================
DOM-COMPARE.JS — DOM, COMPARAÇÃO CSS×JS E TROCA VISUAL DE QUADROS
===============================================================================
Contém renderizações dos últimos quadros e a função switchBoard, peça central da
navegação visual. Mantenha switchBoard genérica para novas aulas poderem usá-la.
===============================================================================
*/

/* DADOS DA AULA 12: operações de comunicação HTML ↔ JavaScript. */
const domTopics = {
  querySelector: {
    label: 'querySelector()',
...
// Atualiza o quadro DOM com sintaxe HTML, JavaScript e resultado do tópico escolhido.

function renderDomTopic(key) {
  const data = domTopics[key];
  if (!data) return;
  QB.toggleActive(domRows, row => row.dataset.dom === key);
...
/* DADOS DA AULA 11: relações conceituais CSS × JavaScript. */
const compareTopics = {
  selectors: {
...
// Abre o detalhe de comparação entre CSS e JavaScript.
...
// Retorna da comparação detalhada para a tabela.

function closeCompareTopic() {
  if (compareDetailView.hidden) return;
  compareDetailView.hidden = true;
  compareTableView.hidden = false;
  QB.restoreFocus(comparePreviousFocus);
}

// Realiza a troca genérica entre duas aulas, incluindo animação, foco, anúncio acessível e ajuste responsivo.

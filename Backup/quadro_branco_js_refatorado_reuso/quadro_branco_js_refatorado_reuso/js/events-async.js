/*
===============================================================================
EVENTS-ASYNC.JS — EVENTOS WEB, UTILITÁRIOS E AJUSTE RESPONSIVO
===============================================================================
Além das demonstrações de eventos, este módulo contém o mecanismo que mede o
conteúdo visível e aplica compactação/escala quando necessário.

ATENÇÃO
Qualquer alteração em fitVisibleLesson/scheduleLessonFit deve ser testada em
celular, tablet, notebook de pouca altura, monitor e TV, pois afeta todas as aulas.
===============================================================================
*/

/* SINTAXES DOS EVENTOS: chave data-event → código exibido. */
const eventSyntax = {
...
/* CONJUNTOS DE RABISCOS DECORATIVOS usados no ciclo automático. */
const scribbleSets = [
  [
...
// Substitui os rabiscos decorativos do quadro de Eventos, opcionalmente com animação de apagar/escrever.
...
// Escolhe o próximo conjunto de rabiscos sem repetir imediatamente o atual.
...
// Agenda a próxima troca automática dos rabiscos quando o ciclo não estiver pausado.
...
// Remove escala/medidas temporárias antes de recalcular a responsividade da aula.
...
// Mede a aula visível e aplica uma redução proporcional somente quando necessário para caber no quadro.
...
// Agrupa/reagenda medições responsivas para evitar cálculos excessivos durante resize e mudanças de conteúdo.
...
// Abre diretamente uma aula a partir do sumário usando o mapa de destinos.
...
// Atualiza a área de log do laboratório de eventos.

function setLog(message) {
  eventLog.innerHTML = `<strong>Status:</strong> ${message}`;
}

// Escapa texto antes de inseri-lo em trechos de código/HTML demonstrativo.

function escapeHTML(value) {
  return value.replace(/[&<>]/g, char => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[char]));
}

// Aplica marcação visual simples à sintaxe dos exemplos sem depender de biblioteca externa.
...
// Exibe a sintaxe correspondente ao evento selecionado.
...
// Atualiza a descrição resumida do evento durante foco/hover.

function previewRow(row) {
  QB.toggleActive(rows, item => item === row);
...
// Monta o laboratório interativo do evento selecionado.
